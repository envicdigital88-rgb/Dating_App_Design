...
import { Avatar, Badge, Skeleton } from './ui/Bits';
...
export function AppShell({ children }: { children?: React.ReactNode }) {
    ...
    return (
      <div className="flex h-screen w-full bg-cream-deep/40">
        <aside className="sticky top-0 hidden h-screen w-[272px] shrink-0 flex-col overflow-y-auto border-r border-sand/70 bg-cream/90 px-5 py-6 lg:flex space-y-4">
           <Skeleton className="h-10 w-40 mb-8 mx-auto rounded-full bg-sand" />
           <Skeleton className="h-10 w-full rounded-2xl bg-white/50" />
           <Skeleton className="h-10 w-full rounded-2xl bg-white/50" />
           <Skeleton className="h-10 w-full rounded-2xl bg-white/50" />
           <Skeleton className="h-10 w-full rounded-2xl bg-white/50" />
        </aside>
        <div className="flex-1 flex flex-col items-center justify-center space-y-4 p-8">
           ...
           <Skeleton className="h-8 w-64 rounded-full bg-sand" />
           <Skeleton className="h-96 w-full max-w-md rounded-[32px] bg-sand/60" />
        </div>
      </div>
    );
  }
  ...
  { to: '/secret-wingles', label: 'Secret Wingles', icon: <GhostIcon className="h-[18px] w-[18px]" /> },
    ...
    <div className="min-h-full w-full bg-transparent">
      ...
      <div className="mx-auto flex w-full max-w-[1400px]">
        ...
        <aside className="sticky top-0 hidden h-screen w-[272px] shrink-0 flex-col overflow-y-auto border-r border-sand/70 bg-cream-deep/40 px-5 py-6 lg:flex">
          <button onClick={() => router.push('/discover')} className="mb-8 flex w-full justify-center">
            <BrandMark />
          </button>
